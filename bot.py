...
def _build_ssl_args() -> dict:
    if _CA_CERT_INLINE:
        tmp = tempfile.NamedTemporaryFile(mode="w", suffix=".pem", delete=False, prefix="aiven_ca_")
        tmp.write(_CA_CERT_INLINE.replace("\\n", "\n"))
        tmp.flush(); tmp.close()
        return {"ssl_ca": tmp.name, "ssl_verify_cert": True, "ssl_verify_identity": False}
    return {"ssl_disabled": False, "ssl_verify_cert": False}


_CONNECT_ARGS = {
    "host": _DB_HOST, "port": _DB_PORT, "user": _DB_USER,
    "password": DB_PASSWORD, "database": _DB_DATABASE,
    "connection_timeout": 15, "autocommit": False,
    **_build_ssl_args(),
}
...
def _get_pool() -> pooling.MySQLConnectionPool:
    global _pool
    if _pool is None:
        _pool = pooling.MySQLConnectionPool(pool_name="ntbot", pool_size=5, **_CONNECT_ARGS)
    return _pool
...
def db_get_batch(platform, course_id):
    con = _conn(); cur = con.cursor(dictionary=True)
    cur.execute("SELECT * FROM batches WHERE platform=%s AND course_id=%s", (platform, course_id))
...
def db_upsert_batch(platform, course_id, channel_id=None, remove_channel=False, status=None):
    con = _conn(); cur = con.cursor()
    cur.execute("INSERT IGNORE INTO batches (platform, course_id) VALUES (%s, %s)", (platform, course_id))
    if remove_channel:
        cur.execute("UPDATE batches SET channel_id=NULL WHERE platform=%s AND course_id=%s", (platform, course_id))
    elif channel_id is not None:
        cur.execute("UPDATE batches SET channel_id=%s WHERE platform=%s AND course_id=%s", (channel_id, platform, course_id))
    if status is not None:
        cur.execute("UPDATE batches SET status=%s WHERE platform=%s AND course_id=%s", (status, platform, course_id))
    con.commit(); cur.close(); con.close()
...
def db_is_sent(platform, course_id, content_id):
    con = _conn(); cur = con.cursor()
    cur.execute("SELECT 1 FROM sent_files WHERE platform=%s AND course_id=%s AND content_id=%s", (platform, course_id, content_id))
    found = cur.fetchone() is not None; cur.close(); con.close()
    return found


def db_mark_sent(platform, course_id, content_id):
    con = _conn(); cur = con.cursor()
    cur.execute("INSERT IGNORE INTO sent_files (platform, course_id, content_id) VALUES (%s,%s,%s)", (platform, course_id, content_id))
    con.commit(); cur.close(); con.close()


def db_reset_sent(platform, course_id):
    con = _conn(); cur = con.cursor()
    cur.execute("DELETE FROM sent_files WHERE platform=%s AND course_id=%s", (platform, course_id))
    con.commit(); cur.close(); con.close()
...
# ═══════════════════════════════════════════════════════════════
def build_course_keyboard(courses: list, platform: str, cfg: dict) -> InlineKeyboardMarkup:
    """
    ...
    if connected:
        keyboard.append([InlineKeyboardButton("━━━━  ✅  CONNECTED  ━━━━", callback_data="noop")])
    ...
    if not_connected:
        keyboard.append([InlineKeyboardButton("━━━  ○  NOT CONNECTED  ━━━", callback_data="noop")])
...
async def show_courses(edit_fn, platform: str):
    ...
    total_connected = sum(1 for c in courses if db_get_batch(platform, c["id"]) and db_get_batch(platform, c["id"]).get("channel_id"))
...
async def cmd_batches(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not platform:
        ...
        await update.message.reply_text("Which platform?", reply_markup=InlineKeyboardMarkup(kb))
        return
...
async def cmd_forceall(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("⚡ Starting force-update for all connected batches…")
...
async def _do_broadcast(reply_fn, app: Application, text: str):
    ...
    for batch in batches:
        try:
            ...
            fail += 1
            fail_details.append(f"  • `{batch['channel_id']}` ({batch['platform']} / {batch['course_id']}): {e}")
...
async def cb_rmchan(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"✅ Channel removed from course `{cid}`.\nUse /start to go back.", parse_mode="Markdown")
...
async def cb_pause(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"⏸ Course `{cid}` paused.\nUse /start to go back.", parse_mode="Markdown")
...
async def cb_resume(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"▶️ Course `{cid}` resumed.\nUse /start to go back.", parse_mode="Markdown")
...
async def cb_restart(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"🔄 Course `{cid}` reset.\nContent re-posts on next scheduled check.", parse_mode="Markdown")
...
async def cb_forceupdate(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not batch or not batch.get("channel_id"):
        await query.edit_message_text(f"⚠️ Course {course_id} has no channel. Set one first."); return
...
async def post_content(app: Application, platform, channel_id, detail, title):
    ...
    try:
        if thumb and thumb.startswith("http"):
            await app.bot.send_photo(chat_id=channel_id, photo=thumb, caption=caption, reply_markup=kb, parse_mode="HTML")
        else:
            await app.bot.send_message(chat_id=channel_id, text=caption, reply_markup=kb, parse_mode="HTML")
    except Exception as e:
    ...
    async with aiohttp.ClientSession() as session:
        for batch in batches:
            ...
            results.append({
                "platform": platform, "course_id": course_id,
                "channel_id": channel_id, "posted": posted,
                "skipped": skipped, "errors": err_msgs,
            })
